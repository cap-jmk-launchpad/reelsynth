...
- **DenoiseOpt meta residual score** — champion ranking now uses prolonged residual-vs-ideal ∈ [0,1] (`score = clamp(1 − residual_rms / max(ideal_rms, ε), 0, 1)`): tile denoised cycle vs `generate_sound_ideal` (no open-wrap). D/S remain report auxiliaries; `FROZEN_THETA` unchanged until a full 1500 re-lock. Sanity: `cargo run -p reelsynth --release --bin bench_denoise_meta -- 40`.
