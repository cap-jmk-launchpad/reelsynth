...
use crate::layout::{
    PIANO_BLACK_HEIGHT_RATIO, PIANO_BLACK_WIDTH_RATIO, PIANO_HEIGHT, PIANO_OCTAVES,
    PIANO_START_NOTE, PIANO_WHITE_KEY_WIDTH,
};
...
impl<'a> PianoKeyboard<'a> {
    ...
    pub fn show(self, ui: &mut Ui) -> (Response, PianoResponse) {
        ...
        let total_w = white_count as f32 * PIANO_WHITE_KEY_WIDTH;
        let size = Vec2::new(total_w, PIANO_HEIGHT);
        ...
        if ui.is_rect_visible(rect) {
            ...
            if ui.input(|i| i.pointer.primary_down()) {
                if let Some(pos) = ui.input(|i| i.pointer.interact_pos()) {
                    if rect.contains(pos) {
                        if let Some(note) = hit_test(pos, rect, &white_notes, &black_notes) {
            ...
            for (i, &note) in white_notes.iter().enumerate() {
                let x = rect.min.x + i as f32 * PIANO_WHITE_KEY_WIDTH;
                let key_rect = Rect::from_min_size(
                    Pos2::new(x, rect.min.y),
                    Vec2::new(PIANO_WHITE_KEY_WIDTH, PIANO_HEIGHT),
                );
                let active = self.keys_down.contains(&note);
                let top = if active { accent_ui } else { Color32::from_rgb(244, 244, 245) };
                ...
                painter.rect_filled(
                    key_rect.shrink2(Vec2::new(0.0, key_rect.height() * 0.15)),
                    4.0,
                    top,
                );
                painter.rect_stroke(
            ...
            let black_h = PIANO_HEIGHT * PIANO_BLACK_HEIGHT_RATIO;
...
fn hit_test(
    ...
    let col = ((pos.x - rect.min.x) / PIANO_WHITE_KEY_WIDTH).floor() as usize;
