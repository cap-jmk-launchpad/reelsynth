...
pub fn process_sample(state: &mut VoiceState, ctx: &VoiceSampleContext<'_>) -> [f32; 2] {
    ...
    for (oi, osc) in ctx.patch.oscillators.iter().enumerate() {
        ...
        let (pan_l, pan_r) = equal_power_pan(osc.pan);
        ...
        for u in 0..unison {
            ...
            *phase += osc_freq / ctx.sr;
            ...
            let osc_sample =
                bank.sample(wt_pos, *phase) * osc.level * amplitude / unison as f32;
    ...
    let base_cutoff = (ctx.patch.filter.cutoff + cutoff_mod).max(25.0);
    let key_cutoff = key_tracked_cutoff(base_cutoff, ctx.freq, ctx.patch.filter.key_tracking);
    let env_cutoff = filter_env_cutoff(key_cutoff, filt_env, ctx.sr);
    let cutoff = env_cutoff.min(ctx.sr * 0.45);
