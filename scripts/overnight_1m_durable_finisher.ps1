...
function Ensure-JobIfDead {
  ...
  if (Test-Path $launcher) {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $launcher -Iters 500000 -MaxHours 240 -HistoryEvery 1 -Device cuda -Seed 1902771841 -AlgoTag "PPO+GA+PBT+NAS+depth+MoE"
  } else {
    if (-not (Test-Path $VenvPy)) { Log "NO_VENV"; return }
    $argList = @(
      (Join-Path $Repo "scripts\overnight_gpu_rl_arch.py"),
      "--iters", "500000", "--device", "cuda", "--max-hours", "240", "--history-every", "1", "--seed", "1902771841", "--pop-size", "12", "--algo-tag", "PPO+GA+PBT+NAS+depth+MoE"
    )
